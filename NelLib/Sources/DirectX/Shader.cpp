...
void Shader::WriteBuffer(UINT slot, void* pData)
{
	if(slot < m_pBuffers.size())
		RENDERER.GetContext()->UpdateSubresource(m_pBuffers[slot], 0, nullptr, pData, 0, 0);
}
void Shader::SetTexture(UINT slot, Texture* tex)
{
	...
	switch (m_kind)
	{
	case Vertex:	RENDERER.GetContext()->VSSetShaderResources(slot, 1, &pTex); break;
	case Pixel:		RENDERER.GetContext()->PSSetShaderResources(slot, 1, &pTex); break;
	}
}
